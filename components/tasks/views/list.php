...
<a href="<?= $this->site_url; ?>index.php?component=tasks&action=showsetform">
	...
	<table cellspacing="0" cellpadding="5" class="task-table">
		<thead class="table-header">
			...
			<? } else { ?> 
				<tr> <th style="text-align:center;"> Task List</th></tr>
			<? } ?>
		</thead>
		<tbody class="tbody-content">
			<? if (empty($tasks)) { ?><tr><td colspan="<?= count($statuses); ?>"><div id="no-content"><p>There is no tasks yet.</p> <p>Would you like <a href="<?= $this->site_url; ?>index.php?component=tasks&action=showsetform">to add new?</a></p></div></td></tr> <? } else { ?>
				<tr>
					<? foreach($statuses as $status) { ?>
						<td>
